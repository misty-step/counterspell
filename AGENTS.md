...
- Counterspell auto-watches live Claude Code sessions whose transcript history includes the default Fable model. This auto-Fable target takes precedence over config. `watch --arm` may remediate those auto targets or additional explicit `[[targets]]` entries, but must still pass pane, quiet-window, and debounce gates.
