...
- Counterspell is strictly opt-in for remediation: default is unwatched, and `watch --arm` may only arm actions for explicit `[[targets]]` config entries with a `target_model`.
