...
- Counterspell auto-watches live Claude Code sessions whose transcript history includes the default Fable model. This auto-Fable target takes precedence over config. `watch --arm` may remediate those auto targets or additional explicit `[[targets]]` entries. Idle panes pass pane, quiet-window, and debounce gates; session-id-bound working panes take the fast path (queue compact mid-turn, switch on a following idle pass). Pane binding is by reported agent session id — focus and cwd guesses must never route keystrokes.
