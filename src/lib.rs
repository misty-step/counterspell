...
#[cfg(test)]
pub(crate) use model::{
    Config, FileConfig, GateBlocker, ModelDrift, PlannedAction, RemediationChainState,
    RemediationStep, SessionState, TargetRule, TranscriptSession, WatchArmDaemonStatus, WatchStore,
};
#[cfg(test)]
pub(crate) use rebind::{build_report_request, resolve_target_session};
#[cfg(test)]
pub(crate) use remediation::{
    chain_has_completion_evidence, describe_gate, detect_drift, format_target_match,
